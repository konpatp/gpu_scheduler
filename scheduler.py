...
class GPUScheduler:
    ...
    def __enter__(self):
        ...
        # In debug mode, disable all health checks by setting thresholds appropriately
        # This makes find_free_gpus() return all non-banned, non-locked GPUs
        if self.debug_mode:
            ...
            memory_threshold = self.memory_free_threshold_gb
            power_threshold = self.power_threshold_w
            utilization_threshold = self.utilization_threshold_percent

        start_time = time.time()
        while time.time() - start_time < self.max_wait_time:
            # Get free GPUs based on free memory, power consumption, and utilization
            # Note: utilization_threshold can be None, which find_free_gpus() supports
            free_gpus = find_free_gpus(memory_threshold, power_threshold, utilization_threshold)  # type: ignore

            if not free_gpus:
                debug_prefix = "🔧 DEBUG: " if self.debug_mode else ""
                logger.info(
                    f"{debug_prefix}No free GPUs available. Waiting {self.poll_interval} seconds..."
                )
            ...
            if self.specific_gpus is not None:
                ...
                if not free_gpus:
                    debug_prefix = "🔧 DEBUG: " if self.debug_mode else ""
                    logger.info(
                        f"{debug_prefix}No free GPUs available from the specified set. Waiting {self.poll_interval} seconds..."
                    )
                    time.sleep(self.poll_interval)
                    continue
            ...
            if len(free_gpus) < self.num_gpus:
                debug_prefix = "🔧 DEBUG: " if self.debug_mode else ""
                logger.info(
                    f"{debug_prefix}Only {len(free_gpus)} GPUs available, but {self.num_gpus} requested. Waiting {self.poll_interval} seconds..."
                )
                time.sleep(self.poll_interval)
                continue
            ...
            for gpu in free_gpus:
                ...
                try:
                    ...
                    debug_prefix = "🔧 DEBUG: " if self.debug_mode else ""
                    logger.debug(f"{debug_prefix}Acquired lock for GPU {gpu_id}")
                    ...
                    logger.info(f"{debug_prefix}GPU {gpu_id} ({gpu_name}) acquired - Memory: {memory_free_gb:.2f}GB free, "
                               f"Power: {power_draw}W, Utilization: {utilization}%")

                except Timeout:
                    # This GPU is locked by another process, try the next one
                    debug_prefix = "🔧 DEBUG: " if self.debug_mode else ""
                    logger.debug(f"{debug_prefix}GPU {gpu_id} is locked by another process")
                    continue
            ...
            if len(acquired_gpu_ids) >= self.num_gpus:
                ...
                if torch.cuda.is_available():
                    ...
                    debug_prefix = "🔧 DEBUG: " if self.debug_mode else ""
                    logger.info(f"{debug_prefix}Acquired {len(self.gpu_ids)} GPUs: {self.gpu_ids}")
                    logger.info(f"{debug_prefix}Set GPU {self.gpu_ids[0]} as the default CUDA device")
                else:
                    debug_prefix = "🔧 DEBUG: " if self.debug_mode else ""
                    logger.warning(f"{debug_prefix}PyTorch CUDA is not available. Cannot set device.")
                ...
                debug_prefix = "🔧 DEBUG: " if self.debug_mode else ""
                logger.debug(
                    f"{debug_prefix}Failed to acquire {self.num_gpus} GPUs. Got only {len(acquired_gpu_ids)}. Retrying..."
                )
        ...
        debug_prefix = "🔧 DEBUG: " if self.debug_mode else ""
        raise TimeoutError(
            f"{debug_prefix}Couldn't acquire {self.num_gpus} GPUs within {self.max_wait_time} seconds"
        )
